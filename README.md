...
| [prompt-optimizer](#prompt-optimizer)       | 작성된 프롬프트를 ai가 이해하기 쉽고 토큰을 절약하게 바꿔줌 |
...
로컬 SLM을 이용하여 사용자가 입력한 프롬프트를 최적화 해줍니다.

### 동작 방식

사용자가 /prompt-optimizer 를 입력하여 명시적으로 호출을 하거나 "최적화 해줘", "프롬프트 요약해줘" 등 프롬프트 최적화 요청을 하면 호출됨
사용자가 작성한 프롬프트를 optimize.py에 넘겨 현재 ollama에 있는 모델중 optimize.py에 설정된 모델(기본: qwen3.5:2b)을 호출하여 프롬프트를 최적화 함.

### 언제 사용하는가

- 작성한 프롬프트가 난해하고 길며 구조가 짜야있지 않을때

### 사용 예시

```
/prompt-optimizer 오늘요 제가요 일을 하다가 이거를 뭐냐 봤는데 이 로직에서 사람이 들어와서 봐야하는데 보지 않고 그냥 바로 멈추는 것도 없이 바로 실행되었어요
```

```
다음 내용을 최적화 해주세요 "오늘요 제가요 일을 하다가 이거를 뭐냐 봤는데 이 로직에서 사람이 들어와서 봐야하는데 보지 않고 그냥 바로 멈추는 것도 없이 바로 실행되었어요"
```

### 출력 결과물

--dry-run: 결과만 보여주고 실행 안 함

"Ensure that the program stops when a human is present, but continue execution if not."  
 Keywords: program, human presence
Original: ~45 tokens → Optimized: ~17 tokens → Savings: ~62%

--compare: 원본 포함 before/after 표시 (유지)
Original (~45 tokens):  
 ▎ 오늘요 제가요 일을 하다가 이거를 뭐냐 봤는데 이 로직에서 사람이 들어와서 봐야하는데 보지 않고 그냥 바로 멈추는 것도 없이 바로 실행되었어요

Optimized (~11 tokens):  
 ▎ "Ensure that the script checks and acknowledges human interaction before proceeding."

Keywords: script, human interaction
